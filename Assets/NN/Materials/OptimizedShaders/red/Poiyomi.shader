Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/ea082618e714cdb41a92728f7f124e70"
{
	Properties
	{
 ...
 #define PROP_DISTORTIONFLOWTEXTURE 
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,5,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * 2 - 1;
				#else
				...
				#endif
				half distortionStrength = float(0.1);
				half distortionStrength1 = float(0.01);
				#ifdef POI_AUDIOLINK
				
				if (AudioLinkIsAvailable() && float(0) && float(1))
				{
					distortionStrength += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, AudioLinkData(uint2(0, uint(float(0)))).r);
					distortionStrength1 += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, AudioLinkData(uint2(0, uint(float(0)))).r);
				}
				#endif
				flowVector *= distortionStrength;
				flowVector1 *= distortionStrength1;
				return poiMesh.uv[float(5)] + ((flowVector.xy + flowVector1.xy) / 2) * distortionMask;
			}
			#endif
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					float holoRim = saturate(1 - smoothstep(min(float(0), float(1)), float(1), poiCam.vDotN));
					holoRim = abs(lerp(1, holoRim, float(0.812)));
					poiFragData.alpha *= float(1) ?1 - holoRim : holoRim;
				}
			}
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(5,5,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1549673,0,1,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(5,5,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1549673,0,1,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1549673,0,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos, float4(10,0,10,10).xyz) * float(5)) + _Time.x * float(5)).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(10,0,10,10).xyz, float(5), float(5), float(5), float(0), pos);
					}
				}
				if (float(0))
				{
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + _Time.x * float4(2,2,0,0)).r;
				#else
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				float2 mainUV = poiMesh.uv[float(7)].xy;
				if (float(0))
				{
					mainUV = sharpSample(float4(1,1,1,1), mainUV);
				}
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(3,3,0,0)) + _Time.x * float4(0,0,0,0));
